        ...
        try {
            ...
            const res = await fetch(apiUrl, {
                ...
                headers: Object.assign({ "Content-Type": "application/json", 
                    // APIキー認証ヘッダーを追加
                    "X-API-Key": process.env.NEXT_PUBLIC_API_KEY || "" }, (idToken ? { Authorization: `Bearer ${idToken}` } : {})),
                body: JSON.stringify({
      ...
      <SidebarProvider>
        ...
        <div className="flex flex-col h-[calc(100vh-4rem)]">
          ...
          <div className="flex-1 overflow-y-auto p-4" data-testid="chat-messages">
            {messages.length === 0 ? (<div className="text-center text-gray-500 mt-8" data-testid="welcome-message">
                ...
                {messages.map((msg, i) => (<div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    : 'bg-gray-100 text-gray-900'}`} data-testid={msg.role === 'user' ? 'user-message' : 'ai-message'}>
          ...
          <div className="border-t p-4" data-testid="chat-input-area">
            <div className="max-w-3xl mx-auto">
              <div className="flex gap-2">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !loading && input.trim().length > 0 && sendMessage()} className="flex-1 border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="ゲームについて質問してください..." disabled={loading} data-testid="message-input"/>
                <button onClick={sendMessage} disabled={loading || input.trim().length === 0} className="px-6 py-2 bg-blue-500 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-blue-600 transition-colors" data-testid="send-button">
                  送信
                </button>
              </div>
            </div>
