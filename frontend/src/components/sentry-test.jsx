...
export function SentryTestComponent() {
    ...
    if (!isMounted) {
        return (<div className="p-4 border border-dashed border-gray-300 rounded-lg m-4 bg-gray-50">
    ...
    return (<div className="p-4 border border-dashed border-orange-300 rounded-lg m-4 bg-orange-50">
      <h3 className="text-lg font-semibold mb-4 text-orange-800">
        🔧 Sentry Test Panel (Development Only)
      </h3>
      <div className="space-y-2 mb-4">
        <button onClick={testClientError} disabled={isLoading} className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 mr-2 disabled:bg-gray-400">
          {isLoading ? '送信中...' : '🔥 Test Error Tracking'}
        </button>
        <button onClick={testMessage} disabled={isLoading} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 mr-2 disabled:bg-gray-400">
          {isLoading ? '送信中...' : '📨 Test Message Logging'}
        </button>
        <button onClick={testUserAction} disabled={isLoading} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600 mr-2 disabled:bg-gray-400">
          {isLoading ? '送信中...' : '👤 Test User Action'}
        </button>
        <button onClick={testDirectSentry} disabled={isLoading} className="px-4 py-2 bg-purple-500 text-white rounded hover:bg-purple-600 mr-2 disabled:bg-gray-400">
          {isLoading ? '送信中...' : '🧪 Test Direct Connection'}
        </button>
        <button onClick={testNetworkConnection} disabled={isLoading} className="px-4 py-2 bg-indigo-500 text-white rounded hover:bg-indigo-600 disabled:bg-gray-400">
          {isLoading ? '送信中...' : '🌐 Test Network Connection'}
        </button>
      </div>
      {testResult && (<div className={`p-3 border rounded text-sm ${testResult.includes('✅')
                ? 'bg-green-100 border-green-300 text-green-800'
                : testResult.includes('❌')
                    ? 'bg-red-100 border-red-300 text-red-800'
                    : 'bg-blue-100 border-blue-300 text-blue-800'}`}>
          {testResult}
          {eventId && (<div className="mt-2 text-xs text-gray-700">Event ID: {eventId}</div>)}
        </div>)}
