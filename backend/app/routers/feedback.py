...
async def submit_feedback(fb: FeedbackCreate = Body(...), auth_info: dict = Depends(require_read_permission)) -> dict:
    try:
...
async def list_recent(limit: int = 50, auth_info: dict = Depends(require_read_permission)) -> dict:
