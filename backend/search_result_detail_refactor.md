...
8. **LLMで「構造化 / ベクトル / ハイブリッド」の分顛を実装**
   ...
   - **やるべきこと（細分化）**
     ...
     3. クエリタイプ（構造化/ベクトル/ハイブリッド）ごとの分岐ロジック実装
        - `HybridSearchService`で分類結果に応じて検索戦略を切り替える分岐処理を実装
        - 例: 構造化→DB検索のみ、ベクトル→ベクトル検索のみ、ハイブリッド→両方実行
     4. 分類結果のテストケース作成
        - 代表的なクエリ例ごとに分類結果が期待通りになるかユニットテストを追加
        - 境界ケース（曖昧なクエリ等）も含めて検証
     5. API/サービス全体の動作確認・テスト
...
10. 返却jsonを一覧表示
   - **やるべきこと（細分化）**
     1. APIレスポンス（詳細jsonリスト）の型定義・型チェック
