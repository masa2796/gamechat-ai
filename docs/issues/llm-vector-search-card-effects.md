...
|-------|------|-----------|----------|-------|------|
| 0 | 初回 (combined 除外) | effect_* | base (≈0.4) | 10 | Precision / MRR 重視（長文バイアス抑制） |
| 1 (plateau注入) | 0件 もしくは top3 が低分散 (stddev ≤ cfg.stddev OR spread ≤ cfg.score_spread) | effect_combined + effect_* | base + cfg.combined_extra_min_score | cfg.combined_top_k (clamped 10-30) | 分散不足や0件時の Recall/多様性補完 |
| 2 (計画) | Stage1 後も0件 | Stage1 + synonym 再埋め込み | (Stage1 min) -0.03〜-0.05 再検討 | 20 | 語彙拡張救済 (zero-hit 更削減) |

Plateau 判定: 検索 Stage0 で得たユニークタイトル上位3件スコア集合 V (|V|≥2) に対し
  stddev(V) ≤ cfg.stddev (既定 0.005) もしくは (max(V)-min(V)) ≤ cfg.score_spread (既定 0.01)
が成立、または 0件 の場合に Stage1 を実行。

Stage1 では combined を無条件常時利用せず「必要時のみ」注入することで combined 長文の過剰優遇による MRR 低下を緩和しつつ、Plateau/0件ケースの Recall を回復させる。
...
### 🔄 第4計測 (Stage0 で combined 除外直後) ※記録
（注: 第4計測単独の数値ログは簡易記録のみ。Stage0 除外により MRR 回復を狙ったが、plateau 未導入段階では P@10 / Recall@10 は据え置きで MRR も大幅改善には至らず。詳細個別ログは `logs/eval/` 参照。正式値は第5計測で確定。）

| Metric | Value (暫定) | 第3比 | コメント |
|--------|--------------|-------|----------|
| P@10 | 0.1625 | ±0.0000 | Precision 変化なし |
| Recall@10 | 0.4198 | ±0.0000 | Recall 横ばい |
| MRR | ~0.335-0.338 | +≈0.001〜0.004 | combined 除外で僅かに回復（長文上位占有減） |
| zero_hit_rate | 0.0000 | ±0.0000 | 継続 0 |

### ♻️ 第5計測 (Plateau 条件付き combined 再投入) 2025-08-30 19:38 JST
コマンド: `python scripts/data-processing/eval_precision_batch.py --real --dump-scores`

| Metric | Value | 第4比 | コメント |
|--------|-------|-------|----------|
| P@10 | 0.1625 | ±0.0000 | Precision 変化なし（再投入条件が限定的でノイズ増加なし）|
...
2. P@10/Recall が横ばい → 追加改善には Stage2 (同義語再埋め込み) または synonym 辞書拡張が必要。
3. MRR 0.38 目標との差分 ~0.042。ranking 微ブースト（短文優位係数 / keyword boost）や combined_extra_min_score 微調整 (+0.02→+0.01) の感度分析が候補。
4. 次計測で plateau_trigger_rate を可視化し再投入条件の過不足を評価。

### 🎛 Plateau 設定パラメータ（config `settings.VECTOR_SEARCH_CONFIG['plateau']`）
| Key | Env | 役割 | 初期値 | 推奨調整レンジ | メモ |
|-----|-----|------|--------|----------------|------|
| enable_combined | VECTOR_ENABLE_COMBINED_PLATEAU | 機能ON/OFF | true | true/false | 本番で一時無効化可能 |
| stddev | COMBINED_PLATEAU_STDDEV | top3 標準偏差閾値 | 0.005 | 0.004–0.006 | 小さいほど厳格 (発火減) |
| score_spread | COMBINED_PLATEAU_SCORE_SPREAD | top3 最大-最小差分閾値 | 0.01 | 0.008–0.012 | stddevとOR条件 |
| combined_extra_min_score | COMBINED_EXTRA_MIN_SCORE | Stage1 注入時の base への加算Δ | 0.02 | 0.01–0.03 | Precision 影響大 (高い=厳格) |
| combined_top_k | COMBINED_PLATEAU_TOP_K | Stage1 の top_k 上限 | 12 | 10–30 | 過剰取得抑制（内部で10-30にクリップ） |
