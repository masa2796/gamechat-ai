...
解消タスク（優先度: 高）
- [ ] クエリ正規化（Synonym/Typo補正）レイヤーを追加（HybridSearchService 直前 or EmbeddingService）
  - 語彙マップ初版:
    - 場所: { フィールド, 盤面, ボード } → 場
    - 対象: { フォロー, フォロワ } → フォロワー
    - リーダー: { 顔, フェイス } → リーダー
    - バウンス: { 手札に戻す, 手札に返す, バウンス } → 手札に戻す
    - ランダム: { 無作為 } → ランダム
  - 軽量ルール例: `/(フォロー)(?!ワ)/ → フォロワー`、全角/半角・長音の正規化。
- [ ] Zero-hit リトライ戦略の実装
