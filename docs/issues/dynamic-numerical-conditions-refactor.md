...
> **ARCHIVE_CANDIDATE**: 動的数値検索はMVPから除外されたため、この改修計画は先送りとします。
...
### Phase 3: プロンプト・テストの改善 【低優先度】 ✅ 完全完了
...
#### 3.1 LLMプロンプトの改善
- [x] `classification_service.py`のシステムプロンプト更新
  - [x] 固定数値例示の削除
  - [x] 集約クエリ例の追加
  - [x] より多様な数値条件例の追加
- [x] 分類精度向上のためのプロンプト調整
- [x] モック環境での集約・複雑数値パターン検出の改善

#### 3.2 テストケースの拡充
- [x] 集約クエリのテストケース追加
  - [x] 最大値/最小値検索のテスト
  - [x] 上位N件検索のテスト
  - [x] 複合条件（集約+フィルター）のテスト
- [x] 複雑数値パターンのテストケース追加
- [x] ランダム数値でのテスト追加
- [x] エラーケースのテスト追加
- [x] 包括的テストファイル `test_aggregation_comprehensive.py` の作成
- [x] パフォーマンステスト、エッジケーステスト、エラーハンドリングテストの追加
- [x] 多言語混在クエリテストの追加
...
- [x] 複数値条件（「50または100」等）に対応
- [x] 近似値条件（「約100」等）に対応
- [x] ClassificationServiceの集約・複雑数値パターン認識の改善
...
- [x] テストカバレッジ90%以上維持
- [x] 既存テストケースの全通過
- [x] 包括的な集約クエリテストケースの追加（22個のテスト）
- [x] ランダム数値テスト、エッジケーステスト、エラーハンドリングテストの実装
